...
void make_jn_tensor( RMatrix Jnmat01, RMatrix Jnmat10, //changes here: newly added routine
		  ...
		  std::vector<RMatrix> &Jntensor)
{
	RMatrix empty(0,0);
	Jntensor.clear();
	Jntensor.push_back(empty);
	Jntensor.push_back(Jnmat01);
	Jntensor.push_back(Jnmat02);
	Jntensor.push_back(Jnmat03);
	Jntensor.push_back(Jnmat10);
	Jntensor.push_back(empty);
	Jntensor.push_back(Jnmat12);
	Jntensor.push_back(Jnmat13);
	Jntensor.push_back(Jnmat20);
	Jntensor.push_back(Jnmat21);
	Jntensor.push_back(empty);
	Jntensor.push_back(Jnmat23);
	Jntensor.push_back(Jnmat30);
	Jntensor.push_back(Jnmat31);
	Jntensor.push_back(Jnmat32);
	Jntensor.push_back(empty);
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void local_fields(double &spin, int &site, int &t, double &hx, double &hy, double &hz,
		      ...
		      double &eff_field_x,
		      double &eff_field_y,
		      double &eff_field_z) //changes here
{
	...
	//}
	//
	//changes belown for next neighbors Recheck again
	for (int j=0;j<nneighbors[site].size();j++)
        {
                int k=nneighbors[site][j];
                int t2=ijkt[k][3];
                int cind=(4*t)+t2;
                eff_field_x+=(Jntensor[cind](0,0)*configx[k])+(Jntensor[cind](0,1)*configy[k])+(Jntensor[cind](0,2)*configz[k]);
                eff_field_y+=(Jntensor[cind](1,0)*configx[k])+(Jntensor[cind](1,1)*configy[k])+(Jntensor[cind](1,2)*configz[k]);
                eff_field_z+=(Jntensor[cind](2,0)*configx[k])+(Jntensor[cind](2,1)*configy[k])+(Jntensor[cind](2,2)*configz[k]);
        }
	std::vector<double> heff=eff_h_field(t,hx,hy,hz,gmats);
	eff_field_x=eff_field_x*spin+heff[0]; eff_field_y=eff_field_y*spin+heff[1]; eff_field_z=eff_field_z*spin+heff[2];
}
		    ...
		    STensor 			      & smunu) //changes here
{
	...
	make_jn_tensor(Jnmat01, Jnmat10, Jnmat02, Jnmat20, Jnmat03, Jnmat30, Jnmat12, Jnmat21, Jnmat13, Jnmat31, Jnmat23, Jnmat32, Jntensor);
        //changes done by adding Jntensor
	...
	for (int intt=0;intt<numtimes;intt++)
	{
		double energyj=total_j_energy(spin, config0x,config0y,config0z,
							 ...
							 Jmat23, Jmat32,
						     	 Jnmat01, Jnmat10,
                                                         ...
                                                         Jnmat12, Jnmat21,
                                                         Jnmat13, Jnmat31,
                                                         Jnmat23, Jnmat32, Jnnn, bond_disorder_matrix, ijkt);//changes here
		...
		fourier_transforms(spin,ijkt,gmats,phases,config0x,config0y,config0z,sxtq,sytq,sztq); // local to global change
		...
		for (int i=0;i<nsites;i++)
		{
			      ...
			      local_fields(spin,i,t,hx,hy,hz,config0x,config0y,config0z,neighbors, nneighbors, Jtensor, Jntensor,gmats,  
					     Jnnn, bond_disorder_matrix, ijkt, eff_field_x, eff_field_y, eff_field_z); //changes here
		...
		for (int i=0;i<nsites;i++)
		{
			      ...
			      local_fields(spin,i,t,hx,hy,hz,config1x,config1y,config1z,neighbors, nneighbors, Jtensor, Jntensor,gmats,  
					     Jnnn, bond_disorder_matrix, ijkt, eff_field_x, eff_field_y, eff_field_z); //changes here
			      cross_product(spin, sign*sx_i, sign*sy_i, sign*sz_i, eff_field_x, eff_field_y, eff_field_z, crossx, crossy, crossz);
			      k2x[i]=crossx;
			      k2y[i]=crossy;
		...
		for (int i=0;i<nsites;i++)
		{
			      ...
			      local_fields(spin,i,t,hx,hy,hz,config2x,config2y,config2z,neighbors, nneighbors, Jtensor, Jntensor,gmats,
					     Jnnn, bond_disorder_matrix, ijkt, eff_field_x, eff_field_y, eff_field_z); //changes here
			      cross_product(spin, sign*sx_i, sign*sy_i, sign*sz_i, eff_field_x, eff_field_y, eff_field_z, crossx, crossy, crossz);
			      k3x[i]=crossx;
			      k3y[i]=crossy;
		...
		for (int i=0;i<nsites;i++)
		{
			      ...
			      local_fields(spin,i,t,hx,hy,hz,config3x,config3y,config3z,neighbors, nneighbors, Jtensor, Jntensor,gmats,
					     Jnnn, bond_disorder_matrix, ijkt, eff_field_x, eff_field_y, eff_field_z); //changes here
			      cross_product(spin, sign*sx_i, sign*sy_i, sign*sz_i, eff_field_x, eff_field_y, eff_field_z, crossx, crossy, crossz);
			      k4x[i]=crossx;
			      k4y[i]=crossy;
