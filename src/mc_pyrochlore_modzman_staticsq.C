		   ...
		   double gxy, double gz) 
{
	...
	{	
		...
		for (int64_t n=0; n<(nsamples+nburn);n++) // Begin MC
		{
			...
			{
				//int numqs=qvals.size();
				fourier_transforms(spin,ijkt,gmats,phases,infos[b].configx,infos[b].configy,infos[b].configz,sxq,syq,szq);
				mcnorm=mcnorm+1;
				//cout<<n<<endl;
	   			for (int nq=0;nq<numqs;nq++) 
	      			{
	...
	sxxij0=2.0*sxxij0/double(mcnorm)/double(nsites);
		...
		ss<<"param2_"<<Jnnn<<"Jnnng_"<<tempKelvin<<"T_"<<npart<<"kpart_"<<mil<<"mil.dat";
